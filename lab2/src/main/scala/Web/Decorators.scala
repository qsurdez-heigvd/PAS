...
object Decorators:
  ...
  /** Wrap an endpoint. Read the request to search for the session id in the
    * cookies. Provide the endpoint the matching `Session` if it exists. Create
    * and provide a new `Session` otherwise.
    */
  class getSession(sessionSvc: SessionService) extends cask.RawDecorator:
    def wrapFunction(
      ...
      val existingSession = ctx.cookies
        .get(SESSION_COOKIE)
        .map(c => c.value)
      ...
      val res = delegate(ctx, Map("session" -> session))

      if existingSession.isEmpty then
        // Add newly created session to the cookies
        res.map(r =>
          r.copy(cookies =
            r.cookies.appended(cask.Cookie(SESSION_COOKIE, session.sid)),
          ),
        )
      else res
