...
/** Allow the retrieval of information linked to a session.
  */
class Session(val sid: String):
  private var _currentUser: Option[String] = None

  /** @return
    *   the pseudo of the current user
    */
  def getCurrentUser: Option[String] = _currentUser

  /** @param user
    *   the pseudo of the new current user
    */
...
/** Allow the creation of a session with an identifier. The identifier allow its
  * later retrieval
  */
...
class SessionImpl extends SessionService:
  ...
  override def create(): Session = 
    val session = Session(nextSid())
    existingSessions(session.sid) = session
    session
